			        ...
			        <tr v-for="(course) in createdCourses" :key="course.id" :data-counter="counter=0">
			            ...
			            <td width="25%"> 	
							<span v-for="(course_experiment,index ) in course.course_experiment" :key="index+'_xc'">
								<a :class="index >1?'experiment bullets text-primary line-h1': 'bullets line-h01 text-primary'" href="#"  @click="checktaskcreated(course_experiment.experiment.id,course_experiment.experiment.page)"> {{course_experiment.experiment.name}}</a>							
							</span>																																	
			            </td>
			            <!-- <td width="15%">500L</td>	 -->		            
			            <td width="10%">
				...
				checktaskcreated:function(id,pagename){					
					if(this.weeklyworks.length >0){						
						...
						if (find>0) {
							...
							location.href =`/${pagename}/${this.weeklyworks[find].id}`;
						}else{
							Swal.fire({
